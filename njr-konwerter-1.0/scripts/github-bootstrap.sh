...
USER_JSON="$(api_get https://api.github.com/user)" || {
  echo "Nie udało się połączyć z api.github.com/user (token wygasł / zły zakres / sieć)." >&2
  echo "Klasyczny PAT: zakres „repo”. Fine-grained: dostęp do repozytoriów + zapis zawartości." >&2
  exit 1
}
TOKEN_USER="$(printf '%s' "$USER_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin)['login'])")"
OWNER="$TOKEN_USER"
if [ -n "$ORG" ]; then
  OWNER="$ORG"
fi
