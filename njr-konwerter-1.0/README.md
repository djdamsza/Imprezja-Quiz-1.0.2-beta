...
| [`GITHUB.md`](GITHUB.md) | GitHub: token, CI build (Win + mac Intel/ARM), linki do release |
...
**Szablon w repozytorium Quiz (VoteBattle):** folder `njr-konwerter-1.0` możesz trzymać tylko jako **wzór do skopiowania** przy zakładaniu osobnego repo NJR. Kod aplikacji nie powinien być częścią repo Quiz — w `.gitignore` projektu Quiz ignorowany jest `editor/*` (z wyjątkiem `editor/README.md`). Po utworzeniu dedykowanego repozytorium usuń stąd ten szablon albo zostaw pusty `editor/` z samym `README.md`.
