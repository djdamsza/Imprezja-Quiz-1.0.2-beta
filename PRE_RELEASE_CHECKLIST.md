...
Przed zbudowaniem i wydaniem nowej wersji IMPREZJA wykonaj poniższe kroki.
...
Z tej wersji biorą się nazwy plików (np. `IMPREZJA Setup 1.0.1.exe`, `IMPREZJA-1.0.1-arm64.dmg`).
...
- Uruchom apl``ikację: `npm start` lub `npm run electron`
- Sprawdź:
...
**Wynik w `dist/`:**
- `IMPREZJA Setup X.Y.Z.exe` – instalator
- `IMPREZJA X.Y.Z-portable.exe` – wersja portable (bez instalacji)
...
```
→ Wynik: `IMPREZJA-X.Y.Z-universal.dmg` i `IMPREZJA-X.Y.Z-universal-mac.zip` (działają na Intel i Apple Silicon).
...
**Wynik w `dist/`:**
- np. `IMPREZJA-X.Y.Z-universal.dmg`, `IMPREZJA-X.Y.Z-x64.dmg`, `IMPREZJA-X.Y.Z-arm64.dmg`
